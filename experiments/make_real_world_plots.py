"""Create figures for real-world-style experiments."""
...
_plot_cache_dir = tempfile.mkdtemp(prefix="tofu-real-world-plot-cache-")
...
def plot_missingness_sweep(results_dir: Path, figures_dir: Path) -> None:
    df = load_csv(results_dir, "real_world_summary.csv")
    datasets = sorted(df["dataset"].unique())
    sns.set_theme(style="whitegrid", context="paper")
    fig, axes = subplot_grid(len(datasets), width=5.0, height=3.8)
    ...
    fig.suptitle("Real-world robustness to missing features", y=1.22, fontsize=12)
    save_figure(fig, figures_dir, "real_world_missingness_sweep")


def aggregate_trajectory(df: pd.DataFrame) -> pd.DataFrame:
    grouped = df.groupby(["dataset", "p", "method", "t"], as_index=False)["cumulative_regret"]
...
def plot_regret_over_time(results_dir: Path, figures_dir: Path) -> None:
    df = load_csv(results_dir, "real_world_trajectories.csv")
    p_value = representative_p(df["p"])
    stats = aggregate_trajectory(df[np.isclose(df["p"], p_value)])
    datasets = sorted(stats["dataset"].unique())

    sns.set_theme(style="whitegrid", context="paper")
    fig, axes = subplot_grid(len(datasets), width=5.2, height=3.8)
    for ax, dataset in zip(axes, datasets):
        panel = stats[stats["dataset"] == dataset]
    ...
    fig.suptitle(f"Real-world regret over time at p = {p_value:g}", y=1.22, fontsize=12)
    save_figure(fig, figures_dir, "real_world_regret_over_time")
...
def plot_rank_diagnostics(results_dir: Path, figures_dir: Path) -> None:
    ...
    if not datasets:
        ...
        ax.axis("off")
        save_figure(fig, figures_dir, "real_world_rank_diagnostics")
        return

    stats = (
        frame.groupby(["dataset", "method", "t"], as_index=False)["active_rank_numeric"]
        .mean()
        .rename(columns={"active_rank_numeric": "mean_rank"})
    )
    sns.set_theme(style="whitegrid", context="paper")
    fig, axes = subplot_grid(len(datasets), width=4.8, height=3.6)
    for ax, dataset in zip(axes, datasets):
        panel = stats[stats["dataset"] == dataset]
    ...
    fig.suptitle(f"Adaptive-rank diagnostics at p = {p_value:g}", y=1.20, fontsize=12)
    save_figure(fig, figures_dir, "real_world_rank_diagnostics")
...
def plot_full_info_refs(results_dir: Path, figures_dir: Path) -> None:
    ...
    handles, labels = axes[-1].get_legend_handles_labels()
    fig.legend(handles, labels, loc="upper center", ncol=4, frameon=False, bbox_to_anchor=(0.5, 1.12))
    fig.suptitle("Real-world full-information references", y=1.22, fontsize=12)
    save_figure(fig, figures_dir, "real_world_full_info_refs")


def plot_fixed_rank_validation(results_dir: Path, figures_dir: Path) -> None:
    df = load_csv(results_dir, "real_world_rank_selection.csv")
    p_value = representative_p(df["p"])
    frame = df[np.isclose(df["p"], p_value)]
    stats = (
        frame.groupby(["dataset", "method_family", "rank"], as_index=False)["final_regret"]
    ...
    handles, labels = axes[-1].get_legend_handles_labels()
    fig.legend(handles, labels, loc="upper center", ncol=2, frameon=False, bbox_to_anchor=(0.5, 1.10))
    fig.suptitle(f"Real-world fixed-rank validation at p = {p_value:g}", y=1.20, fontsize=12)
    save_figure(fig, figures_dir, "real_world_fixed_rank_validation")


def plot_calibration_diagnostics(results_dir: Path, figures_dir: Path) -> None:
    df = load_csv(results_dir, "real_world_calibration.csv")
    p_value = representative_p(df["p"])
    datasets = sorted(df["dataset"].unique())
    sns.set_theme(style="whitegrid", context="paper")
    fig, axes = subplot_grid(len(datasets), width=6.0, height=4.2)

    for ax, dataset in zip(axes, datasets):
        frame = df[(df["dataset"] == dataset) & np.isclose(df["p"], p_value)].copy()
        frame = frame[
        ...
        ax.set_ylabel("Validation final regret")
        ax.set_title(dataset)
    fig.suptitle(f"Real-world calibration diagnostics at p = {p_value:g}", y=1.08, fontsize=12)
    save_figure(fig, figures_dir, "real_world_calibration_diagnostics")
...
def main() -> None:
    ...
    print(f"Wrote real-world figures to {args.figures_dir}")
